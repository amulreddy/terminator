...
public class YoozCommunicationService extends BaseResillience4JClient {
	public void YoozConnection() throws Exception {

		RestTemplate rt = new RestTemplate();

		UriBuilder uriBuilder = UriBuilder
				.fromUri("https://us1.getyooz.com/auth/realms/yooz/protocol/openid-connect/token");
	...
	public <T> T getForObject(UriBuilder uriBuilder, Class<T> clazz, Duration timeout) {
		...
		HttpEntity headers = null;
		return (T) objectMapper.convertValue(
				decorate(() -> restTemplate.exchange(uriBuilder.build(), HttpMethod.GET, headers, type), timeout)
						.getBody(),
				clazz);
	}
